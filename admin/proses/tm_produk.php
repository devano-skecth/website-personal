...
if (move_uploaded_file($tmp_file, "../../image/produk/".$namaGambarBaru)) {
	$no = 0;
	$result = mysqli_query($conn, "INSERT INTO produk VALUES('$kode','$nm_produk','$namaGambarBaru','$desk','$harga','$keb[$no]')");

	$filter = array_filter($kd_Stok);
	$jml = count($filter) - 1;
	$no = 0;
	while ($no <= $jml) {

		mysqli_query($conn, "INSERT INTO bom_produk VALUES('$format','$kd_Stok[$no]','$kode','$nm_produk','$keb[$no]')");
		$no++;
	}
